class ButtonSimple extends Phaser.GameObjects.Sprite {
  constructor(opts) {
    // init
    const { scene, x, y, id, sheet, costume, gelvo, enabled = true, ariaLabel = false } = opts;
    super(scene, x, y, sheet, costume);
  ...
  enterButtonHoverState() {
    if (this.buttonEnabled) {
      ...
      if (this.gelvo) {
        this.scene.sys.game.soundController.playGelVO(this.gelvo);
      }
...
const OVER = '_over';
const DOWN = '_over'; // '_down';
const DISABLED = '_over'; // '_disabled';
